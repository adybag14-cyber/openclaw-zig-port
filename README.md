...
- Recent FS6 progress (2026-03-06):
  ...
  - mailbox wrapper probes now enforce the narrow mailbox-control boundaries directly: invalid `magic` preservation, invalid `api_version` preservation, clean valid recovery after header rejection, stale-seq replay no-op preservation, and a dedicated five-stage `u32` sequence-wraparound family (`baseline`, `pre-wrap state`, `pre-wrap mailbox seq`, `post-wrap state`, `post-wrap mailbox state`) over the same live PVH run
