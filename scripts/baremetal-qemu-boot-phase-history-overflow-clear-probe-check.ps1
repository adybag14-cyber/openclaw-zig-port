...
if ($overflowCount -ne 2 -or $firstSeq -ne 3 -or $lastSeq -ne 66 -or $clearLen -ne 0 -or $restartLen -ne 2 -or $restartFirstSeq -ne 1) {
    throw "Unexpected boot-phase-history overflow/clear values: overflow=$overflowCount first=$firstSeq last=$lastSeq clearLen=$clearLen restartLen=$restartLen restartFirst=$restartFirstSeq"
}
