...
- `FS5.5` TCP/IP first strict slice is now partially reached locally: `src/protocol/ethernet.zig` and `src/protocol/arp.zig` provide Ethernet/ARP framing, `src/pal/net.zig` exposes `sendArpRequest` plus `pollArpPacket`, host regressions prove ARP request loopback over the mock RTL8139 device, and the live QEMU proof `scripts/baremetal-qemu-rtl8139-arp-probe-check.ps1` now proves ARP request transmission, receipt, decode, and TX/RX counter advance over the freestanding PVH artifact. IPv4/UDP/TCP remain open.
