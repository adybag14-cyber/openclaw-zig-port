...
- `FS5.5` storage/disk depth advanced locally: `src/baremetal/storage_backend.zig` now provides the shared backend selector, `src/baremetal/ata_pio_disk.zig` now provides a real ATA PIO `IDENTIFY` / `READ` / `WRITE` / `FLUSH` path, `src/pal/storage.zig` plus `src/baremetal/tool_layout.zig` now route through that shared backend, and hosted/host regressions prove backend preference, identify-backed capacity, ATA mock-device read/write/flush behavior, and ATA-backed export reporting. The remaining closure gap is the first live bare-metal/QEMU disk mutation + readback proof.
