...
- `FS5.5` TCP/IP strict staged slices are now advanced locally: `src/protocol/ethernet.zig` and `src/protocol/arp.zig` provide Ethernet/ARP framing, `src/protocol/ipv4.zig` and `src/protocol/udp.zig` provide IPv4/UDP framing plus checksum handling, `src/protocol/tcp.zig` now provides strict TCP framing plus pseudo-header checksum validation, `src/protocol/dhcp.zig` now provides strict DHCP discover encode/decode, `src/pal/net.zig` exposes `sendArpRequest` / `pollArpPacket`, `sendIpv4Frame` / `pollIpv4PacketStrict`, `sendUdpPacket` / `pollUdpPacketStrictInto`, `sendTcpPacket` / `pollTcpPacketStrictInto`, and DHCP send/poll helpers, host regressions prove ARP, IPv4, UDP, TCP, and DHCP loopback/decode over the mock RTL8139 device, and the live QEMU proofs `scripts/baremetal-qemu-rtl8139-arp-probe-check.ps1`, `scripts/baremetal-qemu-rtl8139-ipv4-probe-check.ps1`, `scripts/baremetal-qemu-rtl8139-udp-probe-check.ps1`, `scripts/baremetal-qemu-rtl8139-tcp-probe-check.ps1`, and `scripts/baremetal-qemu-rtl8139-dhcp-probe-check.ps1` now prove ARP, IPv4, UDP, TCP framing/payload, and DHCP discover framing/decode plus TX/RX counter advance over the freestanding PVH artifact. DNS and full TCP handshake/connection management remain open.
